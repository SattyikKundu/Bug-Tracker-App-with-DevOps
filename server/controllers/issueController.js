...
// GET /issues/my-work
// Returns logged-in user's current ACTIVE assigned work across accessible projects.
export const getMyWork = async (req, res, next) => {
  ...
  try {
   ...
   /* Normal users should only receive assigned issues from projects they can still access.
    * Global admins retain their existing application-wide access.
    */
    ...
    if (currentUser.role !== "admin") {
      ...
      accessibleProjectIds = accessibleProjects.map((project) => project._id);
    }

    const baseFilter = { assigneeId: currentUserId };

    if (accessibleProjectIds !== null) {
      baseFilter.projectId = { $in: accessibleProjectIds };
    }

   /* Dashboard represents CURRENT work rather than historical completed work.
    * Closed issues therefore don't appear in active My Work collection.
    */
    const activeWorkFilter = {
    ...
    const [
      ...
      Issue.find(activeWorkFilter)
        .populate({ path: "projectId", select: "key name archived" })
        .sort({ updatedAt: -1 })
        .limit(5)
        .lean(),
      ...
      Issue.countDocuments({ ...baseFilter, status: "ready_for_review"})
    ]);

    return res.json({
      issues,
      summary: {
        total:          openCount + inProgressCount + readyForReviewCount,
        open:           openCount,
...
export const updateIssue = async (req,res,next)=>{
  ...
  try{
    ...
    const { 
        ...
        severity, 
        assigneeId
    } = req.body||{}; // destructures inputs from request body
